...
create table if not exists public.audit_records (
  ...
  effective_animal text,
  status text not null check (status in ('correct','divergence','reassignment','linked','new_tag','new_tag_conflict','possible_swap','audit_conflict','replacement_chain','tag_not_registered','tag_not_found','tag_without_animal','animal_not_in_base','unconfirmed','suspicious_tag','possible_typo')),
